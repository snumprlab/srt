...
def main(gt_path, pred_path, output_dir, output_path, num_tasks=1, 
         model_name='chatgpt-3.5-turbo', temperature=0, top_p=1.0, 
         max_new_tokens=256,
         resulting_path=None, prompt_ver="none"):
    prompt_template = PROMPT_TEMPLATE
    if prompt_ver and prompt_ver != "none":
        print("PROMPTVER", prompt_ver + '.txt')
        with open(os.path.join(ROOT_PATH, 'test', 'eval', 'eval_prompts', prompt_ver+ '.txt'), "r") as f:
    ...
    print("Predicted data: ", len(pred_data), pred_path)
    data_to_send_list = []
    ...
    if not resulting_path: 
        resulting_path = os.environ.get("RESULTING_PATH", "./eval_results.jsonl") # default resulting path
    print("Saved to", resulting_path)
